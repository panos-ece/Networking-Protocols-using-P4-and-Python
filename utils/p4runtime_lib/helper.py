...
class P4InfoHelper(object):
    ...
    def buildPacketOutEntry(self, payload, packet_metadata):
        packetOut = p4runtime_pb2.PacketOut()
        packetOut.payload = payload
        
        metadata = packetOut.metadata.add()

        metadata.metadata_id = packet_metadata[0].metadata_id
        metadata.value = packet_metadata[0].value

        return(packetOut)

    def buildDigestEntry(self,digestID,nanosec):
        digest_entry = p4runtime_pb2.DigestEntry()
        digest_entry.digest_id = digestID
        #config = digest_entry.config.add()
        digest_entry.config.max_timeout_ns = 0
        digest_entry.config.max_list_size = 1
        digest_entry.config.ack_timeout_ns = 1
