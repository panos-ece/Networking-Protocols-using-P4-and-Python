...
class SwitchConnection(object):
    ...
    def HandleStreamMessageResponse(self, **kwargs):
        for response in self.stream_msg_resp:
            return(response)

    def PacketIn(self, **kwargs):
        for response in self.stream_msg_resp:
            print("I am into it here")
            if response.HasField("packet"):
                print("Packet is set")
            else:
    ...
    def PacketOut(self, packetOut, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
        request.packet.CopyFrom(packetOut)

        self.requests_stream.put(request)
        return(request)
    
    def WriteDigestEntry(self,digest_entry):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.low = 1
    ...
    def ReadDigestList(self, **kwargs):
        for response in self.stream_msg_resp:
            print("I am into it")
            if response.HasField("digest"):
                ...
                #return(None)
                print(type(self.stream_msg_resp))
                #print(response)
                #continue
                #print("Hi")
                #print(response.WhichOneof("update"))
    ...
    def AckDigestList(self, digest_id, list_id, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
